...
/**
 *env_error - Creates an error message for _env
 *		in case of errors.
...
char *env_error(char **args)
{
	...
	hist_count_str = _itoa(history_count);
	if (hist_count_str != NULL)
		return (NULL);

	args--;
	len = _strlen(name) + _strlen(hist_count_str) + _strlen(args[0]) + 45;
